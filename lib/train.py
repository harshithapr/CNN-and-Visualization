...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	...
	for epoch in xrange(max_epochs):
		...
		for iter in xrange(iter_start, iter_end):
			...
			#############################################################################
			# Update the parameters by a forward pass for the network, a backward #
		...
		val_acc = 0
		#############################################################################
		# Compute the training accuracy and validation accuracy, store the    #
		...
		if val_acc > opt_val_acc:
			...
			for layer in model.net.layers:
				for name in layer.params:
					opt_params[name] = layer.params[name]
